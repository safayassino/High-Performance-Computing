...
int main(int argc, char *argv[]) {
    ...
    if (init_mode == 1) {
        ...
        printf("Grid: %dx%d, Steps: %d, Mode: %s, Threads: %d\n",
               grid->rows, grid->cols, steps, 
               evolution == 0 ? "ORDERED" : "STATIC", num_threads);
        
        double start = get_time();
        if (evolution == 0) {
            evolve_ordered(grid, steps);
        } else {
            evolve_static_omp(grid, steps);
        }
        ...
        write_csv_result("results_openmp.csv", "openmp", grid->rows, grid->cols, 
                        steps, evolution, 1, num_threads, elapsed);
